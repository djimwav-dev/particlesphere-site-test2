...
function ParticleSphereContent() {
  ...
  const logoTexture = textures[0]
  const orbitTextures = textures.slice(1)
  ...
  const orbitingImages = useMemo(() => {
    ...
    for (let i = 0; i < IMAGE_COUNT; i++) {
      ...
      images.push({
        ...
        textureIndex: i % orbitTextures.length,
        color: new THREE.Color().setHSL(Math.random(), 0.7, 0.6),
      })
    }

    return images
  }, [IMAGE_COUNT, SPHERE_RADIUS, orbitTextures.length])
    ...
    <group ref={groupRef}>
      ...
      {orbitingImages.map((image, index) => (
        <mesh key={`image-${index}`} position={image.position} rotation={image.rotation}>
          ...
          <meshBasicMaterial map={orbitTextures[image.textureIndex]} opacity={1} side={THREE.DoubleSide} />
        </mesh>
      ))}
